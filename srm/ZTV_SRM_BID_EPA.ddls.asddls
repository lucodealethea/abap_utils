@AbapCatalog.sqlViewName: 'ZBI_SRM_BID_EPA'
@AbapCatalog.compiler.CompareFilter: true
@AccessControl.authorizationCheck: #CHECK
@EndUserText.label: 'Solicitação de Cotação - Empresas Participantes Contato'
@ClientDependent: true
//change the from to zbi_sc_ep if only Published Quotes link is required 
define view ZTV_SRM_BID_EPA as select distinct from crmd_orderadm_h as rfx_header 
inner join bbpc_proc_type_t as proc on rfx_header.client = proc.client and rfx_header.process_type = proc.process_type and proc.langu = 'P' 
inner join bbp_pdhgp as hgp on rfx_header.client = hgp.client and rfx_header.guid = hgp.src_guid and hgp.src_object_type = 'BUS2200'
and hgp.version_type <> 'H' and hgp.version_type <> 'C'
inner join bbp_pdview_bup as bup on hgp.guid = bup.guid_hi and bup.partner_fct =  '00000017'
...
//key hgp.src_object_type,
key rfx_header.process_type,
key rfx_header.description as RFX_DESCRIPTION,
key rfx_header.guid,
